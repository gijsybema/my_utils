...
print(f'Create folder to store hyperparameter tuning results - {outcome}')

# create a new path to store results
PATH_TO_RESULTS_MODEL_SELECTION = os.path.join('/home/cdsw', 'results', 'model_selection')
destination_folder = os.path.join(PATH_TO_RESULTS_MODEL_SELECTION, outcome)
Path(destination_folder).mkdir(parents=True, exist_ok=True)

# specify filename
file_name = f'03_hyperparameter_tuning_results_{outcome}.txt'
...
with open(destination_and_file, 'a') as f:
    f.write(f'{outcome} - Hyperparameter tuning results\n')
...
print(f'Hyperparameter tuning for outcome {outcome} are stored under {destination_and_file}')
...
dump_file = 'final_model_' + outcome + '.pkl'
...
print(f'Saved {est_name} model for outcome {outcome} to {dump_file} in folder {PATH_TO_FINAL_MODEL}')
